...
namespace sage
{
    ...
    const Model& ModelView::GetRlModel() const
    {
        return rlmodel;
    }

    const Mesh& ModelView::GetMesh(int num) const
    {
        assert(num < rlmodel.meshCount);
        return rlmodel.meshes[num];
    }

    BoundingBox ModelView::CalcLocalMeshBoundingBox(const Mesh& mesh, bool& success) const
    {
        success = true;
    ...
    BoundingBox ModelView::CalcLocalBoundingBox() const
    {
    ...
    int ModelView::GetMaterialCount() const
    {
        return rlmodel.materialCount;
    }

    Matrix ModelView::GetTransform() const
    {
        return rlmodel.transform;
    }

    void ModelView::SetTransform(Matrix trans)
    {
        rlmodel.transform = trans;
    }

    Shader ModelView::GetShader(int materialIdx) const
    {
        assert(materialIdx < rlmodel.materialCount);
        return rlmodel.materials[materialIdx].shader;
    }

    void ModelView::SetShader(Shader shader, int materialIdx) const
    {
        assert(materialIdx < rlmodel.materialCount);
        rlmodel.materials[materialIdx].shader = shader;
    }

    void ModelView::SetShader(Shader shader) const
    {
    ...
    const std::string& ModelView::GetKey() const
    {
        return assetKey;
    }

    void ModelMutable::SetTexture(Texture texture, int materialIdx, MaterialMapIndex mapIdx) const
    {
    ...
    /**
     * Generates a gradient with transparency (raylib version does not have transparency)
     */
        ...
        for (int y = 0; y < height; y++)
        {
            for (int x = 0; x < width; x++)
            {
                ...
                auto alpha = static_cast<unsigned char>(
                    static_cast<float>(outer.a) * factor + static_cast<float>(inner.a) * (1.0f - factor));
