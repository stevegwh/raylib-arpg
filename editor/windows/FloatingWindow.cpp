...
#define RAYGUI_WINDOW_CLOSEBUTTON_SIZE 18
    ...
    if (minimized)
    {
        GuiStatusBar({position.x, position.y, size.x, RAYGUI_WINDOWBOX_STATUSBAR_HEIGHT}, title.c_str());

        if (GuiButton({position.x + size.x - RAYGUI_WINDOW_CLOSEBUTTON_SIZE - close_title_size_delta_half,
    ...
    {
        minimized = GuiWindowBox({position.x, position.y, size.x, size.y}, title.c_str());

        // scissor and draw content within a scroll panel
        Rectangle scissor = {0};
        GuiScrollPanel({position.x, position.y + RAYGUI_WINDOWBOX_STATUSBAR_HEIGHT, size.x,
        ...
        if (CheckCollisionPointRec(mousePosition,
                                   {position.x, position.y + RAYGUI_WINDOWBOX_STATUSBAR_HEIGHT, size.x,
                                    size.y - RAYGUI_WINDOWBOX_STATUSBAR_HEIGHT}) && GuiGetState() != STATE_DISABLED)
        {
...
void
FloatingWindow::DrawContent()
{
    if (GuiButton({position.x + 20 + scroll.x, position.y + 50 + scroll.y, 100, 25}, "Button 1"))
    {
        std::cout << "Button 1 clicked" << std::endl;
    }

    GuiButton({position.x + 20 + scroll.x, position.y + 100 + scroll.y, 100, 25}, "Button 2");
    GuiButton({position.x + 20 + scroll.x, position.y + 150 + scroll.y, 100, 25}, "Button 3");
    GuiLabel({position.x + 20 + scroll.x, position.y + 200 + scroll.y, 250, 25}, "A Label");
    GuiLabel({position.x + 20 + scroll.x, position.y + 250 + scroll.y, 250, 25}, "Another Label");
    GuiLabel({position.x + 20 + scroll.x, position.y + 300 + scroll.y, 250, 25}, "Yet Another Label");
}
