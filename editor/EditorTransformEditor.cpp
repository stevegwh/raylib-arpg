...
namespace sage::editor
{
    ...
    void EditorTransformEditor::RestoreSnapshot(const EditorEditState& snapshot)
    {
        ...
        sys->transformSystem->SetPosition(entity, snapshot.originalPosition);
        sys->transformSystem->SetRotation(entity, snapshot.originalRotation);
        sys->transformSystem->SetScale(entity, snapshot.originalScale);
    ...
    void EditorTransformEditor::ApplyFromInspector(
        const entt::entity entity, const TransformField field, const float amount, const bool absolute)
    {
        if (!sys->registry->valid(entity) || !sys->registry->any_of<sgTransform>(entity)) return;

        const auto& transform = sys->registry->get<sgTransform>(entity);
        Vector3 position = transform.GetWorldPos();
        Vector3 rotation = transform.GetWorldRot();
        Vector3 scale = transform.GetScale();
        const Vector3 originalRotation = rotation;
        const Vector3 originalScale = scale;

        const auto setOrAdd = [absolute, amount](float current) {
            return absolute ? amount : current + amount;
        };
        const auto setOrAddClamped = [absolute, amount](float current) {
            return std::max(MIN_SCALE, absolute ? amount : current + amount);
        };

        switch (field)
        {
        case TransformField::PositionX:
            position.x = setOrAdd(position.x);
            sys->transformSystem->SetPosition(entity, position);
            updateEntityCollisionBounds(entity);
            notify(entity);
            return;
        case TransformField::PositionY:
            position.y = setOrAdd(position.y);
            sys->transformSystem->SetPosition(entity, position);
            updateEntityCollisionBounds(entity);
            notify(entity);
            return;
        case TransformField::PositionZ:
            position.z = setOrAdd(position.z);
            sys->transformSystem->SetPosition(entity, position);
            updateEntityCollisionBounds(entity);
            notify(entity);
            return;
        case TransformField::RotationX:
            rotation.x = setOrAdd(rotation.x);
            break;
        case TransformField::RotationY:
            rotation.y = setOrAdd(rotation.y);
            break;
        case TransformField::RotationZ:
            rotation.z = setOrAdd(rotation.z);
            break;
        case TransformField::ScaleX:
            scale.x = setOrAddClamped(scale.x);
            break;
        case TransformField::ScaleY:
            scale.y = setOrAddClamped(scale.y);
            break;
        case TransformField::ScaleZ:
            scale.z = setOrAddClamped(scale.z);
            break;
        }

        const bool rotationChanged = rotation.x != originalRotation.x || rotation.y != originalRotation.y ||
                                     rotation.z != originalRotation.z;
        const bool scaleChanged =
            scale.x != originalScale.x || scale.y != originalScale.y || scale.z != originalScale.z;

        if (pivotMode == PivotMode::LocalCenter && sys->registry->any_of<Renderable>(entity) &&
            (rotationChanged || scaleChanged))
        {
            const auto& renderable = sys->registry->get<Renderable>(entity);
            if (const auto* model = renderable.GetModel(); model != nullptr)
            {
                const Matrix oldEntityMatrix = BuildRenderableEntityMatrix(position, originalRotation, originalScale);
                const Matrix oldWorldMatrix = MatrixMultiply(model->GetTransform(), oldEntityMatrix);
                Matrix delta = MatrixIdentity();

                if (rotationChanged)
                {
                    const Matrix rotationDelta = MatrixMultiply(
                        MatrixMultiply(
                            MatrixRotateZ((rotation.z - originalRotation.z) * DEG2RAD),
                            MatrixRotateY((rotation.y - originalRotation.y) * DEG2RAD)),
                        MatrixRotateX((rotation.x - originalRotation.x) * DEG2RAD));
                    delta = MatrixMultiply(delta, rotationDelta);
                }

                if (scaleChanged)
                {
                    const auto scaleFactor = [](const float next, const float current) {
                        return std::fabs(current) > 0.0001f ? next / current : 1.0f;
                    };
                    const Matrix scaleDelta = MatrixScale(
                        scaleFactor(scale.x, originalScale.x),
                        scaleFactor(scale.y, originalScale.y),
                        scaleFactor(scale.z, originalScale.z));
                    delta = MatrixMultiply(delta, scaleDelta);
                }

                const Matrix desiredWorldMatrix =
                    MatrixMultiply(oldWorldMatrix, BuildPivotDeltaMatrix(PivotWorldPosition(entity), delta));
                applyWorldMatrix(entity, desiredWorldMatrix, position, rotation, scale);
                notify(entity);
                return;
            }
