	...
	Model ResourceManager::DynamicModelLoad(const std::string& path)
	{
		...
		else
		{
			...
			for (size_t i = 0; i < model.materialCount; ++i)
			{
				...
				memcpy(model.materials[i].shader.locs, oldModel.materials[i].shader.locs, RL_MAX_SHADER_LOCATIONS * sizeof(int));

				// Deep copy maps
				model.materials[i].maps = (MaterialMap*)RL_MALLOC(MAX_MATERIAL_MAPS * sizeof(MaterialMap));
				memcpy(model.materials[i].maps, oldModel.materials[i].maps, MAX_MATERIAL_MAPS * sizeof(MaterialMap));
