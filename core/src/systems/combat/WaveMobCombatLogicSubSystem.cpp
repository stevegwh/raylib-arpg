...
void WaveMobCombatLogicSubSystem::Update(entt::entity entity) const
{
	...
	if (c.target == entt::null || !c.inCombat) return; // TODO: If not in combat, should be removed from system? (But then how can it react to hits?)
    // The answer is obvious. Get a view of CombatableActor AND CombatableStateComponent (or something). Just remove the CombatableStateComponent to remove
    // this from the system.
    // Make a call to the state machine to transition and pass it a combat state component that is generated by a factory.
