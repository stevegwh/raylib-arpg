...
namespace sage
{
    ...
    void ProcessMeshRange(Model& model, const ModelAnimation& anim, int frame, int startMesh, int endMesh, std::vector<bool>& meshUpdated)
    {
    ...
    void UpdateModelAnimation(Model model, ModelAnimation anim, int frame)
    {
        ...
        // Calculate the number of meshes each thread should process
        int meshesPerThread = model.meshCount / nbThreads;
        int remainingMeshes = model.meshCount % nbThreads;

        int startMesh = 0;
        for (unsigned int idThread = 0; idThread < nbThreads; idThread++)
        {
            int endMesh = startMesh + meshesPerThread + (idThread < remainingMeshes ? 1 : 0);
            
            threads.emplace_back(ProcessMeshRange, std::ref(model), std::ref(anim), frame, startMesh, endMesh, std::ref(meshUpdated));
