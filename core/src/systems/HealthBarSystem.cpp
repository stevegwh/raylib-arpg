...
void HealthBarSystem::Draw3D()
{
    ...
    view.each([this](const auto& c, const auto& col)
	{
		...
		billboardPos.y += 1.0f;
		Rectangle sourceRec = { 0.0f, 0.0f, static_cast<float>(c.healthBarTexture.texture.width), static_cast<float>(-c.healthBarTexture.texture.height) };
		DrawBillboardRec(*camera->getRaylibCam(), c.healthBarTexture.texture, sourceRec, billboardPos, {1.0f, 1.0f}, WHITE);
