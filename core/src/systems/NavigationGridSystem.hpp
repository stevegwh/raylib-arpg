...
namespace sage
{
	...
	public:
		...
		void MarkSquareOccupied(const BoundingBox& occupant, bool occupied,
		...
		std::vector<Vector3> AStarPathfindHelper(const entt::entity& entity, const GridSquare& startGridSquare,
		                                         const GridSquare& finishGridSquare, const GridSquare& minRange,
		                                         const GridSquare& maxRange, const GridSquare& extents,
		                                         Heuristic heuristic)
		{
			std::vector<std::vector<bool>> visited(maxRange.row, std::vector<bool>(maxRange.col, false));
			std::vector<std::vector<GridSquare>> came_from(maxRange.row,
			                                               std::vector<GridSquare>(maxRange.col, {-1, -1}));
