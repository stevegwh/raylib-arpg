...
void main()
{
    ...
    vec2 distortionUV = uv + vec2(seconds * 0.5, 0);
    
    // Sample the distortion texture with animated UV
    vec2 distortion = texture(texture0, distortionUV).rg * 2.0 - 1.0;
    ...
    vec3 col = vec3(3.0, 2.0, 1.0);
    d = sin(d * 8.0 + seconds * 8.0) / 8.0;
    d = abs(d);
    d = 0.035 / d;
    
    // Incorporate distortion into the color
    col *= d;
    col += vec3(distortion, 0.0) * 0.2;  // Add some of the distortion to the color
